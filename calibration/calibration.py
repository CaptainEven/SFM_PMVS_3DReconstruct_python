...
def Zhang():
    ...
    # objp包含的是10*7每一角点的坐标
    obj_pt = np.zeros((np.prod(chessboard_size), 3), np.float32)  # 9*6个三维坐标
    obj_pt[:, :2] = np.mgrid[0:chessboard_size[0], 0:chessboard_size[1]].T.reshape(-1, 2)
    ...
    for img_path in tqdm(calibration_paths):
        ...
        if ret == True:
            ...
            # 执行亚像素级角点检测
            corners_refine = cv2.cornerSubPix(gray, corners, (5, 5), (-1, -1), criteria)
    ...
    for i in range(len(calibration_paths)):
        print('frame {:d} rvects:\n'.format(i), rvecs[i])
        print('frame {:d} tvects:\n'.format(i), tvecs[i])
    # return True
    ...
    print("Mean error: {:.5f} pixel".format(tot_error / len(obj_points)))

    np.save(params_dir + '/ret.npy', ret)
    np.save(params_dir + '/dist.npy', dist)
    np.save("../calibration/camera_params/rvecs", rvecs)
    np.save("../calibration/camera_params/tvecs", tvecs)
